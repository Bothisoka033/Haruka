	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							welcome = await getBuffer(`https://api.lolhuman.xyz/api/base/welcome?name=${encodeURI(anu_user)}&member=${encodeURI(mdata.participants.length)}&groupName=${encodeURI(mdata.subject)}&ppuser=${ppmem3}&bgurl=${background}&apikey=Fiktod`)
							try{
							...
							goodbye = await getBuffer(`https://api.lolhuman.xyz/api/base/leave?name=${encodeURI(anu_user)}&member=${encodeURI(mdata.participants.length)}&groupName=${encodeURI(mdata.subject)}&ppuser=${ppmem3}&bgurl=${background}&apikey=Alphabot`)
							try{ 
